...
func (m testErrorMessage) Validate() error { return errTesting }

var (
	// A control error returned by mock functions to emulate a failure.
	errTesting = errors.New("test error")
	...
	testTransportBodyError = roundTripperFunc(func(r *http.Request) (*http.Response, error) {
		return &http.Response{
			...
			ProtoMajor: r.ProtoMajor,
			ProtoMinor: r.ProtoMinor,
			Body:       io.NopCloser(readFunc(func(b []byte) (int, error) { return 0, errTesting })),
			Request:    r,
		}, nil
	})
...
func TestEnqueue(t *testing.T) {
	tests := map[string]struct {
		...
		"group": {
			strings.TrimSpace(fixture("test-enqueue-group.json")),
		...
		"page": {
			strings.TrimSpace(fixture("test-enqueue-page.json")),
			Page{Name: "A", UserId: "B"},
		},

		"screen": {
			strings.TrimSpace(fixture("test-enqueue-screen.json")),
			Screen{Name: "A", UserId: "B"},
		},

		"track": {
			strings.TrimSpace(fixture("test-enqueue-track.json")),
			Track{
				Event:  "Download",
				UserId: "123456",
...
func TestEnqueuingCustomTypeFails(t *testing.T) {
	...
	if err.Error() != "messages with custom types cannot be enqueued: *acore.customMessage" {
...
func TestTrackWithInterval(t *testing.T) {
	...
	var ref = strings.TrimSpace(fixture("test-interval-track.json"))
...
func TestTrackWithTimestamp(t *testing.T) {
	var ref = strings.TrimSpace(fixture("test-timestamp-track.json"))
...
func TestTrackWithMessageId(t *testing.T) {
	var ref = strings.TrimSpace(fixture("test-messageid-track.json"))
...
func TestTrackWithContext(t *testing.T) {
	var ref = strings.TrimSpace(fixture("test-context-track.json"))
...
func TestTrackMany(t *testing.T) {
	var ref = strings.TrimSpace(fixture("test-many-track.json"))
...
func TestClientEnqueueError(t *testing.T) {
	...
	if err := client.Enqueue(testErrorMessage{}); err != errTesting {
